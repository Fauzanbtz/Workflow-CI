...
def train_and_log_model(X_train, X_test, y_train, y_test):
    ...
    # ✅ Gunakan active run dari MLflow CLI jika sudah ada
    active_run = mlflow.active_run()

    if active_run:
        print(f"⚙️ Menggunakan active run dari MLflow CLI (run_id: {active_run.info.run_id})")
        model = RandomForestRegressor(n_estimators=100, random_state=42, n_jobs=-1)
        model.fit(X_train, y_train)
    else:
        print("🚀 Membuat MLflow run baru...")
    ...
    # 🔹 Jika run sudah aktif (mode mlflow run .)
    y_pred = model.predict(X_test)
    rmse = np.sqrt(mean_squared_error(y_test, y_pred))
    r2 = r2_score(y_test, y_pred)

    print("\n📊 HASIL EVALUASI MODEL (MLflow CLI Mode)")
    print(f"RMSE: {rmse:.2f}")
    print(f"R² Score: {r2:.4f}")
